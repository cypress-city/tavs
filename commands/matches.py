...
class MatchCog(commands.Cog):
    ...
    async def quickplay_command(self, inter: discord.Interaction, opponent: discord.User, course: str):
        ...
        p2 = Player(opponent)
        game = Match(p1, p2, inter.channel, QUICKPLAY_RULESET, set_track=course)
        ready_view = ReadyView(p1, p2)
        await message.edit(embed=game.ready_embed(), view=ready_view)
        ...
        game.start_round()
        round_message = await inter.channel.send(embed=game.round_embed(), view=round_view)
        warning_message = None
        warning_sent = False
        while game.time_remaining() and not (p1.quit or p2.quit):
            await round_view.wait()
            round_view = RoundView(p1, p2, timeout=10)
            await round_message.edit(embed=game.round_embed(), view=round_view)
            if game.time_remaining() < 130 and not warning_sent:
                warning_message = await inter.channel.send(embed=construct_embed(title="2 minute warning!", color=RED))
                warning_sent = True

        if warning_message is not None:
            await warning_message.delete()
        await round_message.edit(embed=game.round_embed(), view=None)
        round_over_view = RoundOverView(p1, p2, timeout=180)
        ending_message = await inter.channel.send(embed=game.round_ending_embed(), view=round_over_view)
        ...
        winner = p1 if p1.time == game.winning_time() else p2
        return await inter.channel.send(embed=construct_embed(
